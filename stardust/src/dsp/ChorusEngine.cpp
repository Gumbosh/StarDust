...
void ChorusEngine::prepare(double newSampleRate, int /*samplesPerBlock*/)
{
    ...
    // 2-pole Butterworth LP pre-filter at ~3.4 kHz (12dB/oct — matches Juno-60 CN-2 board filter)
    // Protects MN3009 from aliasing at its ~70 kHz effective sampling rate
    {
...
float ChorusEngine::readInterpolated(float delaySamples) const
{
    ...
    const float p0 = delayBuffer[static_cast<size_t>(i0)];
    const float p1 = delayBuffer[static_cast<size_t>(i1)];
    const float p2 = delayBuffer[static_cast<size_t>(i2)];
    const float p3 = delayBuffer[static_cast<size_t>(i3)];
...
void ChorusEngine::process(juce::AudioBuffer<float>& buffer)
{
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        // 2-pole LP pre-filter (12dB/oct, ~3.4 kHz) — Juno-60 CN-2 board anti-alias
        {
        ...
        monoIn = std::tanh(monoIn * 1.15f) * 0.8697f;

        // Write to mono delay buffer
        delayBuffer[static_cast<size_t>(writePos)] = monoIn;
        ...
        for (int m = 0; m < kNumModes; ++m)
        {
            ...
            const float delayL = std::max(1.0f, v.baseSamples + lfo * v.depthSamples + jL);
            const float delayR = std::max(1.0f, modeIIinCombined
                                              ? v.baseSamples + lfo * v.depthSamples + jR   // non-inverted
                                              : v.baseSamples - lfo * v.depthSamples + jR); // inverted

            float sL = readInterpolated(delayL);
            float sR = readInterpolated(delayR);
