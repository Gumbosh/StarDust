...
juce::AudioProcessorValueTreeState::ParameterLayout StardustProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        ...
        juce::NormalisableRange<float>(3.0f, 24.0f, 0.001f), 12.0f));
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        ...
        juce::NormalisableRange<float>(-12.0f, 12.0f, 0.1f, 1.71f), 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
                                     ...
                                     juce::MidiBuffer& /*midiMessages*/)
{
    ...
    for (int slot = 0; slot < 4; ++slot)
    {
        ...
        switch (fx)
        {
            case 1: // CRUSH — bit + sample-rate degradation
            {
                ...
                const float effectiveRate = juce::jlimit(250.0f, 96000.0f, destroyFaderVal);
            ...
            case 4: // TAPE
            {
                ...
                const float wobble = *apvts.getRawParameterValue("tapeWow") * 0.48f;
                tapeEngine.setWow(wobble);
                tapeEngine.setFlutter(wobble);
    ...
    // internal processing bugs (e.g. J-A instability, granular feedback runaway).
    for (int ch = 0; ch < numChannels; ++ch)
    {
        auto* data = buffer.getWritePointer(ch);
        for (int i = 0; i < numSamples; ++i)
